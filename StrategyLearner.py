...
class StrategyLearner(object):  		  	   		  	  			  		 			 	 	 		 		 	 		 		 	 		  	 	 			  	 
    ...
    def __init__(self, verbose=False, impact=0.0, commission=0.0):  		  	   		  	  			  		 			 	 	 		 		 	 		 		 	 		  	 	 			  	 
        ...
        self.qLearner =  qLearner.QLearner(QLearner(num_states=100,  
                      num_actions=4,  
                      alpha=0.2,  
                      gamma=0.9,  
                      rar=0.98,  
    ...
    def add_evidence(  		  	   		  	  			  		 			 	 	 		 		 	 		 		 	 		  	 	 			  	 
        ...
        upper, lower= getBollingerBands(pricesDF_norm,syms)
        momentum= getMomentum(pricesDF_norm,syms)
        daily_returns_df = getDailyReturns(pricesDF_norm)
        
        indicators_df = pd.concat((price_sma, upper, lower, momentum), columns = ["PRICE_SMA", "UPPER_BOL_BAND", "LOWER_BOL_BAND", "MOMENTUM"],axis=1)
        # indicators_df = indicators.loc[sd_original:]
        
        #get the initial state by discretizing indicators
        initial_state = self.discretize(indicators_df)
        
        while not self.hasConverged():
            holdings = 0
            for day, date in enumerate(pricesDF_norm.index):
                if date == prices.index[0]:
                     ...
                     reward =  holdings * ((curr_price / prev_price) - 1)* (1 - self.impact)
    ...
    def getInitialState(self, indicators_df):
        price_sma_integer = discretize(price_sma)
        upper_bol_band_integer = discretize(upper_bol_band)
        momentum_integer = discretize(momentum)
        
    def discretize(self, data):
        "PRICE_SMA", "UPPER_BOL_BAND", "LOWER_BOL_BAND", "MOMENTUM"
        ...
        stepsize = data.size/10
        data.sort_values(by=[])
        for i in range(0, 10):
            threshold[i] = data[(i+1)*10]
